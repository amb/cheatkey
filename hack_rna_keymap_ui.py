...
__all__ = ("draw_entry", "draw_km", "draw_kmi", "draw_filtered", "draw_hierarchy", "draw_keymaps")
...
from . import process_keys

# The panel for the addon
def addon_draw(layout, context):
    box = layout.box()
    ...
    r.prop(
        ...
        "cheatkey_show_ui",
        text="",
        icon="DISCLOSURE_TRI_DOWN" if pref.cheatkey_show_ui else "DISCLOSURE_TRI_RIGHT",
        emboss=False,
    )
    r.label(text="Batch Edit")

    if pref.cheatkey_show_ui:
        r = box.row()
        r.prop(pref, "cheatkey_search_group", text="Groups")
        r.prop(pref, "cheatkey_search_name", text="Name")
        r.prop(pref, "cheatkey_search_key", text="Keybind")

        # parse groups (separated with comma)
        groups = pref.cheatkey_search_group.split(",")

        new_r = box.column(align=True)
        bind_count, it = process_keys.load_names(
            group=groups,
            search_term=pref.cheatkey_search_name,
            search_bind=pref.cheatkey_search_key,
        )
        r = new_r.row()
        r.label(text="Binds: " + repr(bind_count))
        prev_group = ""
        found_ops = set()
        for i in it:
            # New group def
            if prev_group != i[0]:
                new_r = box.column(align=True)
                r = new_r.row()
                r.box().label(text=i[0])
                prev_group = i[0]
                found_ops = set()

            r = new_r.row()
            # cas = process_keys.cas_string(i[2], i[3], i[4])
            # if cas != "":
            #     cas = "(" + cas + ") "
            kprs = ""
            if i[6].properties is not None:
                ipri = i[6].properties.keys()
                if len(ipri) > 0:
                    kprops = [
                        x + ": " + str(getattr(i[6].properties, x)) for x in ipri if x != "rna_type"
                    ]
                    kprs += " (" + ", ".join(kprops) + ")"

            r.label(text=i[1] + kprs)

            if i[2]:
                r.label(text="", icon="EVENT_C")
            else:
                r.label(text="", icon="BLANK1")
            if i[3]:
                r.label(text="", icon="EVENT_A")
            else:
                r.label(text="", icon="BLANK1")
            if i[4]:
                r.label(text="", icon="EVENT_S")
            else:
                r.label(text="", icon="BLANK1")
            r.label(text=i[5])
...
def draw_keymaps(context, layout, addon=False):
